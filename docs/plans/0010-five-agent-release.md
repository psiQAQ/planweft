...
实施中，正式 `0.4.0` 未发布。公开候选仍为 `0.4.0-rc.5`（next），latest 仍是 RC1。当前分支 `fix/rc6-release-blockers` 准备 `0.4.0-rc.6`，尚未冻结或发布。
...
- 当前动作：上游迁移回归已恢复 721 Passed / 63 Skipped；分批提交后冻结准确 RC6，先验证 Claude 自动采用及 Codex/Claude 停止归因，再补正常信任、权限拒绝与 OpenCode 冷读。
