...
实施中，正式 `0.4.0` 未发布。公开候选为 `0.4.0-rc.10`，源码 `e79e08548c31ed8833da032f68766b36078779f4`，准确 npm SHA-256 `fed540a7a27d9ce8e58d8535bbd1479b612ca0db8e2e62df7b9e9d9e875e8114`。三系统 Check `34349393062`、OIDC `34350081143` 和官方 registry 下载逐字核对 Passed；`latest` 仍是历史 RC1，不能视为稳定发布。

- RC10 Claude 自动维护与冷读 Passed；独立维护 Failed：读取禁读宿主设置、使用固定项目外 scratch、将未运行旧测试记为 Passed。计划采用修复有效，独立冷读 Passed 不弥补 owner 失败。
- RC10 Pi 在模型前的 doctor 因原生相对来源误报而 Failed；冷读仅单独记录，不构成维护交接通过。RC11 已补相对来源最小修复和独立审查，准确包原生复验待完成。
- RC11 将实际任务范围放入可提取目标段，补充本地化/混合标题限制；源码审查与纯函数 LF/CRLF 对照 Passed，不能据此声称模型行为或原生注入已通过。
- DSH RC10 模型组正在运行；OpenCode 直连具体授权已获用户补充，等待串行执行。此前自动审批拒绝作为历史保留，不再是待批准项。
- RC9 的四宿主语义失败、Codex 两轮原始 Failed 与独立派生去重 Passed 均保留；RC8 五宿主生命周期/A-B fixture 等既有成绩不替代当前或稳定准确包。
- 准确稳定包全门槛、各宿主原生去重、部分失败恢复、最终远端升级回退与实际会话未完成，不提升 latest 或创建正式 Release。
