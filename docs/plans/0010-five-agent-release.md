...
实施中，正式 `0.4.0` 未发布。公开 npm 候选为 `0.4.0-rc.4`（next）；准确归档 SHA-256 `c6f54319befc8c43c559fd4c5af2eb6ca3d1b626e6c0b3fd65cb67c11d9d318b`。latest 仍指向 RC1，不能据该标签视为稳定验收通过。

- 已完成：历史脱敏、OIDC 发布及远端逐字核对、五容器 npm 升降级/卸载、Codex/Claude 公开 Git 同提交刷新、三系统安装器 CI。
- 模型与交接：Codex 维护/冷读通过独立审查；Pi 首次记录问题经明确反馈修正并通过新冷读。DSH 指定反馈修正和新冷读也已独立 Passed；OpenCode 冷读验证状态误报仍未解决。所有首次失败保留。
- 发布阻塞：Codex 普通停止复验 Passed；gated 续跑模型正常结束但无计数变化和后续响应，正在核查宿主原因。Claude 明确端点授权已完成，RC4 代码维护与冷读 Passed，但主 Skill 未读取、PWF 计划未采用（独立 Failed），停止溯源六项因收集器解析限制均 Failed；Pro 实际读取 Skill 后仍未采用 PWF，外层中断、冷读 Not Run。OpenCode 冷读准确性问题及最终稳定归档的五平台完整门槛未完成。
- 下一步：本轮反馈审查与证据归档已完成，代码及文档按功能分批提交；先冻结准确 RC5 包并验证 Codex 修复；补齐 Claude 采用/停止溯源和正常信任/权限门槛后，再补准确稳定包和真实远端会话，才可提升 latest、创建稳定 Release 及同步发布分支。
