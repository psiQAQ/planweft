...
RC7 is published to next through [OIDC](https://github.com/psiQAQ/planweft/actions/runs/34333285542), with identical remote bytes and passing non-model native install/remove/reinstall checks in all five images. [Three-OS installer CI](https://github.com/psiQAQ/planweft/actions/runs/34334806556) also passed with Node pinned to 24.20.0; this does not prove live Agent models on Windows/macOS.

Independent RC7 maintenance/cold-read results: Codex Passed; Pi Failed (no plan adoption, prohibited config read, unsupported cold-read attribution); OpenCode Failed (current/historical implementation contradiction missed by its cold reader); DSH core functionality/handoff Passed but overall Failed for host-environment metadata inspection outside the authorized scope, with no observed credential, business-data or old-chat content. RC8 entry decisions and record checks are being prepared. Explicit Claude-compatible endpoint authorization and model acceptance remain pending. The table below retains historical version scopes, not final stable acceptance. See [release status: 中文](releasing.md) / [English](releasing.en.md).
...
The latest candidate RC8 passed three-system CI and install/remove/reinstall in five images. Pi/OpenCode automated maintenance passed, but independent scope or record checks failed. DSH review also found unsupported test-execution claims and missing temporary-path evidence. Additional Claude model authorization is still pending. RC9 addresses the concrete findings; stable 0.4.0 has not passed the release gate.
