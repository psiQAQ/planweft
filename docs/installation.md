[简体中文](installation.md) | [English](installation.en.md)

项目介绍：[中文](../README.md) / [English](../README.en.md) · 跨平台设计：[中文](platforms.md) / [English](platforms.en.md)

<!-- Generated from overlays/program-design/install/INSTALL.md; edit the source. -->
...
以下 `/abs/repo` 是本源码仓库根，`/abs/package` 是选定的 `dist/<host>/program-design`，`/abs/project` 是目标项目，均需换成实际绝对路径。仓库包含六种平台各自识别的 marketplace catalog，名称均为 `program-design`。**marketplace 注册源使用 `/abs/repo`**；平台目录是插件 payload，不能一律用作 marketplace。只有实际包含自有 catalog 的独立包才支持额外的目录注册路线。
...
只启用一套规划执行 hooks。安装 Program Design 不会卸载 PWF；检查旧的独立 Skill、项目 hooks 与其他来源的同名插件。先验证 `project-docs` 实际被读取和当前计划路径，再在已授权维护任务中初始化计划。安装成功、当前会话加载、hooks 信任是不同状态。
...
仓库 `.agents/plugins/marketplace.json` 指向 Codex 包；`plugins/program-design/` 是保留的兼容镜像，不是另一套手工维护源码。
...
卸载用 `codex plugin remove program-design@program-design`，再用 `codex plugin list` 核对。0.2.0 来源可能是 `program-design@personal` 或 `program-design@program-design-local`；按列表中的实际旧 ID 移除，再安装新 ID，避免重复 hooks。只在没有其他插件依赖时移除旧 marketplace。
...
安装/更新后按提示 `/reload-plugins` 或新建会话，核对 `/program-design:project-docs`。单次开发可用 `claude --plugin-dir /abs/repo/dist/claude/program-design`，不与长期安装同时启用。Git marketplace 发布后可将注册源换成真实仓库；每次发布需要递增插件版本以刷新缓存。默认不启用 marketplace 自动更新。[官方 marketplace](https://code.claude.com/docs/en/plugin-marketplaces)、[插件管理](https://code.claude.com/docs/en/discover-plugins)

## Pi

`dist/pi/program-design` 本身是 `pi-package`；根 `package.json` 声明 Skill 和 Extension。在目标项目执行：
...
`-l` 写项目 `.pi/settings.json`；省略则写用户 `~/.pi/agent/settings.json`。本地路径只记录引用、不复制文件，因此来源目录须保留；更新该目录后 `/reload` 或重启。卸载使用 `pi remove -l /abs/repo/dist/pi/program-design`，用户安装省略 `-l`。
...
固定版本用 `npm:@scope/program-design-pi@VERSION`，升级时再次 `pi install -l` 指定新版本。当前官方 `pi update` 单独执行更新 Pi 本体；`pi update --extensions` 更新包，定向更新如上。npm/Git 安装由 Pi 处理包依赖；本地包不要求用户运行开发测试。[官方 packages](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/packages.md)

加载后检查 `/skill:project-docs` 和 `/pd-plan-status`。显式 `/pd-plan-execute` 才启用注入与执行循环，`/pd-plan-execute reset` 恢复被动状态。配置名仍为 `planningWithFiles`，`PWF_MODE` 支持 `auto`、`parity`、`cache-safe`、`notify`；执行循环受平台限制。

## OpenCode V1

`dist/opencode/program-design` 是带预编译 `dist/index.js` 的 npm 包，本地名为 `opencode-program-design`。普通安装不需要编译 TypeScript；本地复制路线需在插件自己的目录安装运行依赖。

1. 将完整包复制到项目 `.opencode/packages/opencode-program-design/`。
...
4. 在项目 `.opencode/plugins/program-design.ts` 写入以下 loader；包内 `install/loader-template.ts` 也提供绝对 file URL 模板。二者选一种，不重复注册。

```typescript
export { PlanningWithFiles } from "../packages/opencode-program-design/dist/index.js"
```

需要命令时将包内 `commands/pd-*.md` 复制到 `.opencode/commands/`。用户 scope 改用 `~/.config/opencode/` 下同样的 `packages/`、`plugins/`、`skills/`、`commands/` 层级。重启后核对 `project-docs`、`pd_init` / `pd_status` / `pd_check`，以及可选 `/pd-pwf`、`/pd-pwf-status`。

本地更新时替换完整独立包和 Skill/命令副本，在独立包重新安装锁定运行依赖后重启。卸载只移除本次的 loader、独立包、Skill 和命令，保留共享 `.opencode`。插件的资产来自包内目录；目标项目是计划写入位置，二者不能互相替代。

npm 发布完成后，原生配置可使用 `"plugin": ["@scope/program-design-opencode@VERSION"]`；升级显式改成新版本并重启，卸载删除该配置项及手工安装的 Skill/命令。沿用既有配置只增删本插件条目。该路线与手工 loader 二选一。npm 路线的运行依赖由宿主包管理器安装。npm 插件代码的加载不等于 Skill 发现；仍将相同版本完整 `skills/project-docs/` 部署到上述发现目录。[官方 plugins](https://opencode.ai/docs/plugins/)、[Skills](https://opencode.ai/docs/skills/)
...
重新启动 Hermes 后检查 Skill 与原生命令。插件启用与 capability grants 分别处理，更新增加能力时由宿主再次询问。手工复制的本地包更新方式是替换以上两个本插件目录；`plugins update` 不负责拉取任意本地副本。卸载使用 `hermes plugins remove program-design`，并移除本次单独复制的 Skill。
...
`hermes plugins update program-design` 拒绝移动 pinned plugin。`hermes skills install` 不提供同等 `--ref` pin；可复现安装应从相同 SHA 的独立 checkout 复制 `skills/project-docs/`，记录 Skill 与 plugin SHA 配对，升级时同步替换。不能用一个未固定的 Skill Hub 导入代替此配对。[官方 plugin 安装、更新与卸载](https://hermes-agent.nousresearch.com/docs/user-guide/features/plugins/)

项目级插件另需 `HERMES_ENABLE_PROJECT_PLUGINS=1`，从项目启动并明确启用；Desktop 使用用户级路线。`pre_verify` 的接续受宿主触发条件与次数限制，不等于原生 Stop 拒绝。

## Cursor

本包使用 `.cursor-plugin/plugin.json`。本地开发安装将完整 `dist/cursor/program-design/` 放到 `~/.cursor/plugins/local/program-design/`，随后 Developer: Reload Window 或重启，在 Customize 检查。更新时替换该完整目录；卸载时移除本插件目录并重载。不要另复制旧项目 hooks 造成重复执行。
...
本地安装后保留来源目录。取得新版本后 `gemini extensions update program-design`，版本判断依赖本地源 manifest 变化；开发时可用 `gemini extensions link /abs/repo/dist/gemini/program-design`。本地 source 不接受 `--ref` 或 `--auto-update`。新会话核对 `/skills list`；卸载用 `gemini extensions uninstall program-design`。
...
本地目录 marketplace 的 path 插件原位加载，更新源目录后 `/restart` 或新会话，不必强求复制缓存更新。Copilot CLI 1.0.83 的本地 path 卸载表现为禁用并保留源文件及 disabled 记录；不再使用整个来源时，再执行 `copilot plugin marketplace remove program-design` 移除其 listing。也可直接 `copilot plugin install /abs/repo/dist/copilot/program-design`；不要与 catalog 来源重复装。插件安装没有可类推的 project `--scope`，官方该参数用于文件/URL 的单 Skill 安装。检查当前用户/profile；CLI 结果不能推定 IDE/Coding Agent 也已通过。[官方 CLI plugin reference](https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-plugin-reference)
...
下面 `REGISTERED_NAME` 必须替换成列表中的实际注册名：本地目录名为 `program-design` 时通常是 `program-design`；隔离验证的源目录名为 `source`，实际 ID 是 `program-design@source`。
