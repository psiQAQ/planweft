...
- The current P1 release checks are defined by the [version policy file](https://github.com/psiQAQ/planweft/blob/master/release/support-policy-0.7.0.json); this deterministic static/logic scope does not replace real host or model regression, which remains `Not Run` for this release.
...
Product status and acceptance evidence are separate dimensions. “Supported” identifies the integrations targeted by the current primary installation and documentation path. “0.4.0 formal core” only says that the exact 0.4.0 archive completed that historical acceptance scope. The deterministic P1 result for 0.7.0 does not prove equivalent behavior in a real host or model environment.
