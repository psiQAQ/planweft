...
PlanWeft 0.5.1 的 `dist/manifest.json` 列出 15 个宿主分发目标。当前产品层面将 `codex`、`claude`、`pi`、`opencode` 和 `dsh` 作为主要支持集成，其余目标按实验性适配处理。分发目标表示包为该宿主准备了资源，不表示宿主已经发现、信任、启用或实际加载这些资源。
...
- 当前版本的发布检查以[版本策略文件](https://github.com/psiQAQ/planweft/blob/master/release/support-policy-0.5.1.json)为准；静态包检查不代替真实宿主回归。
...
产品状态和验收证据是两个不同维度。表中的“主要支持”描述当前安装和文档面向的核心集成；“0.4.0 正式核心”仅说明该精确归档曾完成对应范围的正式验收。0.5.1 是否在某个真实环境中满足同样行为，需要使用当前版本重新验证，不能由历史结果自动推出。
