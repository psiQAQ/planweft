#!/bin/bash
# planweft: User prompt submit hook for Codex
...
if [ -n "${PWF_PLAN_ROOT:-}" ]; then
    case "${PWF_PLAN_ROOT}" in
        /*|[A-Za-z]:[\\/]*) ;;
        *) echo "[planweft] PWF_PLAN_ROOT must be an absolute local path; nothing injected."; exit 0 ;;
    esac
    PIN_PYTHON="$(trusted_python explicit 2>/dev/null)" || PIN_PYTHON=""
    [ -n "$PIN_PYTHON" ] || { echo "[planweft] PWF_PLAN_ROOT could not be validated; nothing injected."; exit 0; }
    ...
    [ -n "$PIN_REAL" ] || { echo "[planweft] PWF_PLAN_ROOT must stay within the current workspace; nothing injected."; exit 0; }
    if [ -n "$PIN_REAL" ] && [ -d "$PIN_REAL" ]; then
        PWF_PLAN_ROOT="$PIN_REAL"
        export PWF_PLAN_ROOT
        PLAN_PREFIX="${PIN_REAL}/"
    else
        echo "[planweft] PWF_PLAN_ROOT is not a directory; nothing injected."
        exit 0
    fi
fi
...
case "$PWF_SESSION_ADMISSION" in
    ...
    legacy) ;;
    *)
        echo "[planweft] Session isolation is armed (${PLAN_PREFIX}.planning/sessions/ exists) and this session is not attached, so no plan was injected. Attachment sentinels use a fixed-width digest of host, canonical project, and PWF_SESSION_ID; delete the sessions directory to return to legacy single-session mode."
        exit 0
        ;;
esac
...
if [ -f "$PLAN_FILE" ]; then
    ...
    echo "[planweft] ACTIVE PLAN — current state:"
    printf '%s\n' "$PLAN_CONTEXT"
    echo ""
    ...
    echo ""
    echo "[planweft] Read findings.md for research context. Continue from the current phase."
fi
exit 0
