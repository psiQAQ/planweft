#!/bin/sh
# planweft: plan-doctor — one-pass self-check for the mechanisms
    ...
    if [ -z "${OUT}" ]; then
        ...
        case "${OUT}" in
            *'===BEGIN-PWF-DATA'*)
                ...
                ;;
            *'[PLAN TAMPERED'*)
                warn "injection: plan is attested but the hash mismatches — run /pw-plan-attest (or scripts/attest-plan.sh) to re-approve the current plan"
                ;;
...
if [ -n "${ATT}" ]; then
    ...
    info "attestation: none (opt-in in legacy mode; default-on in v3 modes; run /pw-plan-attest after approving the plan)"
fi
