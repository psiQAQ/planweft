#!/bin/bash
# program-design: User prompt submit hook for Codex
...
if [ -n "${PWF_PLAN_ROOT:-}" ]; then
    case "${PWF_PLAN_ROOT}" in
        /*|[A-Za-z]:[\\/]*) ;;
        *) echo "[program-design] PWF_PLAN_ROOT must be an absolute local path; nothing injected."; exit 0 ;;
    esac
    PIN_PYTHON="$(trusted_python explicit 2>/dev/null)" || PIN_PYTHON=""
    [ -n "$PIN_PYTHON" ] || { echo "[program-design] PWF_PLAN_ROOT could not be validated; nothing injected."; exit 0; }
    PIN_REAL=$("$PIN_PYTHON" -I -c 'import sys; from pathlib import Path; sys.path.insert(0, sys.argv[1]); import codex_hook_adapter as a; root=a.effective_plan_root(Path.cwd()); print(root or "")' "$HOOK_DIR" 2>/dev/null) || PIN_REAL=""
    [ -n "$PIN_REAL" ] || { echo "[program-design] PWF_PLAN_ROOT must stay within the current workspace; nothing injected."; exit 0; }
    if [ -n "$PIN_REAL" ] && [ -d "$PIN_REAL" ]; then
        PWF_PLAN_ROOT="$PIN_REAL"
        export PWF_PLAN_ROOT
        PLAN_PREFIX="${PIN_REAL}/"
    else
        echo "[program-design] PWF_PLAN_ROOT is not a directory; nothing injected."
        exit 0
    fi
fi
...
if [ ! -e "${PLAN_PREFIX}.planning/sessions" ] && [ ! -L "${PLAN_PREFIX}.planning/sessions" ]; then
    ...
    if [ -z "$PWF_PYTHON" ]; then
        ...
        PWF_SESSION_ADMISSION=$("$PWF_PYTHON" -I -c 'import sys; from pathlib import Path; sys.path.insert(0, sys.argv[1]); import codex_hook_adapter as a; root=a.effective_plan_root(Path.cwd());
...
case "$PWF_SESSION_ADMISSION" in
    ...
    legacy) ;;
    *)
        echo "[program-design] Session isolation is armed (${PLAN_PREFIX}.planning/sessions/ exists) and this session is not attached, so no plan was injected. Attachment sentinels use a fixed-width digest of host, canonical project, and PWF_SESSION_ID; delete the sessions directory to return to legacy single-session mode."
        exit 0
        ;;
esac
...
if [ -f "$PLAN_FILE" ]; then
    ...
    echo "[program-design] ACTIVE PLAN — current state:"
    printf '%s\n' "$PLAN_CONTEXT"
    echo ""
    ...
    echo ""
    echo "[program-design] Read findings.md for research context. Continue from the current phase."
fi
exit 0
