...
> PlanWeft 0.4.0 仍处于候选验收阶段；稳定 0.4.0 须通过 Codex、Claude Code、Pi、OpenCode、DSH 五个平台真实验收。见[发布清单：中文](docs/releasing.md) / [English](docs/releasing.en.md)。
...
**0.4.0 候选系列** 提供一个 `planweft` npm 包，包含安装 CLI、Pi Extension、OpenCode V1 入口及 15 个平台资源目录（含 DSH 原生 bundle）；另有 6 种原生 marketplace 入口和 Gemini/Hermes Git 发布树准备工具。安装方法见[中文指南](docs/installation.md) / [English guide](docs/installation.en.md)，平台能力与验证范围见[中文设计](docs/platforms.md) / [English design](docs/platforms.en.md)。
