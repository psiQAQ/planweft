/**
 * OpenCode plugin entry for program-design.
 *
 ...
 * - tools pd_init, pd_status, pd_check for the model
 */
import { tool } from "@opencode-ai/plugin";
import * as crypto from "node:crypto";
import { ambiguityNotice, buildContext, checkComplete, compactionNote, effectiveProjectRoot, evaluateGate, initPlan, planRootIsPinned, resolvePlan, summarizeStatus, REMINDER, VERSION, WRITE_LIKE_TOOLS, } from "./core.js";
export const PlanningWithFiles = async ({ client, directory }) => {
    const env = process.env;
    const MAX_SESSIONS = 512;
    const sessionDirs = new Map();
    const sessionIsChild = new Map();
    const gateInFlight = new Set();
    ...
    async function sessionInfo(sessionID) {
        ...
        if (cachedDir !== undefined)
            return { dir: cachedDir, child: sessionIsChild.get(sessionID) ?? false, known: true };
        try {
    ...
    function toolRoot(project) {
        if (env.PLANNING_DISABLED === "1")
            return { ok: false, error: "PLANNING_DISABLED=1 is set for this session; program-design is switched off." };
        const root = effectiveProjectRoot(project, env);
        if (!root)
            return { ok: false, error: `PWF_PLAN_ROOT=${env.PWF_PLAN_ROOT} does not resolve to an existing directory.` };
        return root;
    }
    function locate(project) {
        if (env.PLANNING_DISABLED === "1")
            return { root: null, planDir: null, conflicts: [] };
        const root = effectiveProjectRoot(project, env);
        if (!root)
            return { root: null, planDir: null, conflicts: [] };
    ...
    return {
        "chat.message": async (input, output) => {
            try {
                ...
                const alreadyInjected = output.parts.some((part) => {
                    ...
                    return candidate.type === "text" && typeof candidate.text === "string" && candidate.text.startsWith("[program-design] ");
                });
                if (alreadyInjected)
                    return;
                output.parts.push({
        ...
        "tool.execute.after": async (input, output) => {
            try {
                if (!WRITE_LIKE_TOOLS.has(input.tool))
                    return;
                const located = locate(await sessionRoot(input.sessionID));
                if (!located.planDir)
                    return;
        ...
        "experimental.session.compacting": async (input, output) => {
            try {
                const located = locate(await sessionRoot(input.sessionID));
                if (!located.planDir || !located.root)
                    return;
        ...
        event: async ({ event }) => {
            ...
            if (!sessionID || gateInFlight.has(sessionID))
                return;
            try {
                const info = await sessionInfo(sessionID);
                if (!info.known || info.child)
                    return;
        ...
        tool: {
            pd_init: tool({
                description: "program-design: create task_plan.md, findings.md and progress.md. A name creates an isolated .planning/YYYY-MM-DD-<slug>/ plan and makes it active; mode autonomous or gated writes the v3 markers and attests the plan.",
                args: {
            ...
            pd_status: tool({
                description: "program-design: summarize the active plan (id, mode, attestation, current phase, phase counts).",
                args: {},
            ...
            pd_check: tool({
                description: "program-design: report whether every phase of the active plan is complete.",
                args: {},
                async execute(_args, context) {
                    const root = toolRoot(context.directory);
                    if (typeof root !== "string")
