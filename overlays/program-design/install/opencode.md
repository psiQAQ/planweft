Use `dist/opencode/program-design/`, a precompiled **OpenCode V1** package.
The local package is named `opencode-program-design`; no public npm package is
assumed to exist.

1. Copy the complete package to your project's
   `.opencode/packages/opencode-program-design/`.
...
3. Copy its complete `skills/project-docs/` directory to
   `.opencode/skills/project-docs/`. Optionally copy `commands/pd-*.md` to
   `.opencode/commands/`.
4. Create `.opencode/plugins/program-design.ts` with:
...
5. Restart OpenCode. Check `project-docs` and `pd_init` / `pd_status` / `pd_check`.
...
real scope to `@scope/program-design-opencode`. Explicitly change that version to
