...
def metadata(version, description):
    return {'name': PRODUCT, 'version': version, 'description': description,
            'author': {'name': 'Program Design contributors'}, 'license': 'MIT'}
...
def gemini(files, version, description):
    ...
    for event in ['SessionStart', 'BeforeAgent', 'AfterTool', 'PreCompress']:
        hook = {'name': 'program-design-' + event.lower(), 'type': 'command',
    ...
    for name, action in [('pd-plan', 'Continue the authorized implementation using the installed project-docs skill.'),
                         ('pd-plan-status', 'Read the selected project plan and report its status without modifying files.')]:
...
def hermes(files, version):
    result = shared(files, version)
    result.update(flatten(files, '.hermes/plugins/program-design/'))
...
def opencode(files, version, description):
    result = shared(files, version)
    result.update({p: value for p, value in flatten(files, '.opencode/packages/opencode-program-design/').items()
                   if p != 'README.md'})
...
''' + text[end:]
    ...
    result['install/loader-template.ts'] = (
        ...
        b'export { PlanningWithFiles } from "file:///ABSOLUTE/PATH/TO/program-design/dist/index.js"\n', 0o644)
    return result
