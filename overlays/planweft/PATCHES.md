# PlanWeft 本地差异清单（0.2.0 基线与 0.3.0 增量）
...
| --- | --- | --- | --- |
| PD-P01 | manifest、package.json/lock、commands、平台脚本和测试中的产品/Skill身份 | 产品 `planweft`、主 Skill `project-docs`、`pw-` 命令、`pw_` OpenCode tools；保留来源链接、`PWF_*`/`PLAN_ID`/磁盘协议。覆盖反斜杠、编码的 Windows launcher、原生注册及 fallback | 原始/迁移 pytest、Pi/OpenCode；本地身份与 Windows 命令解码契约 |
