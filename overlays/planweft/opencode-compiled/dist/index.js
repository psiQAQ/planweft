...
export const PlanningWithFiles = async ({ client, directory }) => {
    ...
    function toolRoot(project) {
        if (env.PLANNING_DISABLED === "1")
            return { ok: false, error: "PLANNING_DISABLED=1 is set for this session; planweft is switched off." };
    ...
    return {
        "chat.message": async (input, output) => {
            try {
                ...
                const alreadyInjected = output.parts.some((part) => {
                    ...
                    return candidate.type === "text" && typeof candidate.text === "string" && candidate.text.startsWith("[planweft] ");
                });
                if (alreadyInjected)
                    return;
                output.parts.push({
        ...
        tool: {
            ...
            pw_status: tool({
                description: "planweft: summarize the active plan (id, mode, attestation, current phase, phase counts).",
                args: {},
            ...
            pw_check: tool({
                description: "planweft: report whether every phase of the active plan is complete.",
                args: {},
                async execute(_args, context) {
                    const root = toolRoot(context.directory);
                    if (typeof root !== "string")
