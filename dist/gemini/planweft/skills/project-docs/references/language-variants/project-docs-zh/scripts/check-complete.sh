...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planweft] 未找到 task_plan.md — 没有进行中的规划会话。"
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[planweft] 所有阶段已完成（$COMPLETE/$TOTAL）。如果用户有额外工作，请在开始前于 task_plan.md 中新增阶段。"
else
    echo "[planweft] 任务进行中（$COMPLETE/$TOTAL 个阶段已完成）。停止前请更新 progress.md。"
    if [ "$IN_PROGRESS" -gt 0 ]; then
        echo "[planweft] $IN_PROGRESS 个阶段仍在进行中。"
    fi
    if [ "$PENDING" -gt 0 ]; then
        echo "[planweft] $PENDING 个阶段待处理。"
    fi
fi
exit 0
