...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planweft-es] No se encontró task_plan.md — no hay sesión de planificación activa."
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[planweft-es] Todas las fases completadas ($COMPLETE/$TOTAL). Si el usuario tiene trabajo adicional, añadir fases en task_plan.md antes de comenzar."
else
    echo "[planweft-es] Tarea en progreso ($COMPLETE/$TOTAL fases completadas). Actualizar progress.md antes de detenerse."
    if [ "$IN_PROGRESS" -gt 0 ]; then
        echo "[planweft-es] $IN_PROGRESS fases aún en progreso."
    fi
    if [ "$PENDING" -gt 0 ]; then
        echo "[planweft-es] $PENDING fases pendientes."
    fi
fi
exit 0
