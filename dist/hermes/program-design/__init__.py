...
from .tools import (
    program_design_check_complete,
    program_design_init,
    program_design_status,
)

SLASH_COMMANDS = ("pd-pwf", "pd-pwf-status", "pd-plan-status")
...
def _format_status(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"program-design status ({payload.get('plan_id', 'root')}, {payload.get('mode', 'legacy')} mode"
...
def pwf_command(raw_args: str = "") -> str:
    """/pd-pwf [--autonomous|--gated] [--template analytics] [plan name]"""
    try:
    ...
    while index < len(tokens):
        ...
        if token in {"--autonomous", "-a"}:
            ...
            return (
                "Usage: /pd-pwf [--autonomous|--gated] [--template analytics] [plan name]\n"
    ...
    if not result.get("ok", True):
        return f"program-design: {result.get('error', 'initialization failed')}"
    created = ", ".join(result.get("created") or []) or "none (all present)"
    lines = [
        f"program-design: plan ready ({result.get('plan_id', 'root')}, {result.get('mode', 'legacy')} mode)",
...
def status_command(raw_args: str = "") -> str:
    """/pd-pwf-status and /pd-plan-status: compact planning state for the current project."""
    project_dir = _command_cwd()
    payload = json.loads(program_design_status(cwd=str(project_dir)))
...
def _register_commands(ctx: Any) -> None:
    ...
    try:
        register(
            name="pd-pwf",
            handler=pwf_command,
            description="Start program-design: create the plan files (root or .planning/<slug>), optional --autonomous / --gated",
            args_hint="[--gated] [plan name]",
        )
        register(
            name="pd-pwf-status",
            handler=status_command,
            description="Show the active program-design plan: phase counts, mode, attestation",
        )
        register(
            name="pd-plan-status",
            handler=status_command,
            description="Alias of /pd-pwf-status",
        )
...
def register(ctx: Any) -> None:
    ctx.register_tool(
        name="program_design_init",
        toolset="terminal",
        schema={
            "name": "program_design_init",
            "description": (
                "Create program-design markdown files (task_plan.md, findings.md, progress.md) "
        ...
        handler=lambda args, **kw: program_design_init(
            template=args.get("template", "default"),
            cwd=args.get("cwd", ""),
            name=args.get("name", ""),
            mode=args.get("mode", ""),
        ),
        description="Initialize program-design state files.",
    )
    ctx.register_tool(
        name="program_design_status",
        toolset="terminal",
        schema={
            "name": "program_design_status",
        ...
        handler=lambda args, **kw: program_design_status(cwd=args.get("cwd", "")),
        description="Show program-design status summary.",
    )
    ctx.register_tool(
        name="program_design_check_complete",
        toolset="terminal",
        schema={
            "name": "program_design_check_complete",
            "description": "Run the program-design completion check for the active plan.",
            "parameters": {
                "type": "object",
                "properties": {
                    "cwd": {"type": "string", "description": "Target project directory. Defaults to current working directory."},
        ...
        handler=lambda args, **kw: program_design_check_complete(cwd=args.get("cwd", "")),
