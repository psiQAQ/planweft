...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[program-design-de] task_plan.md nicht gefunden — keine aktive Planungssitzung."
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[program-design-de] Alle Phasen abgeschlossen ($COMPLETE/$TOTAL). Wenn der Benutzer zusätzliche Arbeit hat, neue Phasen in task_plan.md hinzufügen, bevor du beginnst."
else
    echo "[program-design-de] Aufgabe läuft ($COMPLETE/$TOTAL Phasen abgeschlossen). progress.md vor dem Stoppen aktualisieren."
    if [ "$IN_PROGRESS" -gt 0 ]; then
        echo "[program-design-de] $IN_PROGRESS Phasen noch in Bearbeitung."
    fi
    if [ "$PENDING" -gt 0 ]; then
        echo "[program-design-de] $PENDING Phasen ausstehend."
    fi
fi
exit 0
