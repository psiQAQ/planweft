...
SESSION_PLAN_BINDING_NOTICE = (
    "[program-design] Multiple plans are available while session isolation "
...
def build_user_prompt_context(project_dir: Path, plan_dir: Path | None = None) -> str:
    ...
    parts = ["[program-design] ACTIVE PLAN — current state:"]
    if plan_dir != project_dir:
        parts.append(f"[program-design] plan: {plan_id_for(project_dir, plan_dir)}")
    try:
        parts.append(
            verified_frame(
                "plan",
        ...
        return f"[program-design] context blocked: {exc}"
    try:
    ...
    try:
        ...
        parts.append("[program-design] Read findings.md for research context. Continue from the current phase.")
...
def post_tool_call(**kwargs: Any) -> None:
    ...
    if project_dir is None or plan_dir is None:
        return None
    message = "[program-design] Update progress.md with what you just did. If a phase is now complete, update task_plan.md status."
