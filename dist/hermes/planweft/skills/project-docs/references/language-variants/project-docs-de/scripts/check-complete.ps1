...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[planweft-de] task_plan.md nicht gefunden — keine aktive Planungssitzung.'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[planweft-de] Alle Phasen abgeschlossen (' + $COMPLETE + '/' + $TOTAL + '). Wenn der Benutzer zusätzliche Arbeit hat, neue Phasen in task_plan.md hinzufügen, bevor du beginnst.')
} else {
    Write-Host ('[planweft-de] Aufgabe läuft (' + $COMPLETE + '/' + $TOTAL + ' Phasen abgeschlossen). progress.md vor dem Stoppen aktualisieren.')
    if ($IN_PROGRESS -gt 0) {
        Write-Host ('[planweft-de] ' + $IN_PROGRESS + ' Phasen noch in Bearbeitung.')
    }
    if ($PENDING -gt 0) {
        Write-Host ('[planweft-de] ' + $PENDING + ' Phasen ausstehend.')
    }
}
exit 0
