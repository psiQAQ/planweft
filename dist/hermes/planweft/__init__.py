...
from .tools import (
    planweft_check_complete,
    planweft_init,
    planweft_status,
)

SLASH_COMMANDS = ("pw-pwf", "pw-pwf-status", "pw-plan-status")
...
def _format_status(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"planweft status ({payload.get('plan_id', 'root')}, {payload.get('mode', 'legacy')} mode"
...
def pwf_command(raw_args: str = "") -> str:
    """/pw-pwf [--autonomous|--gated] [--template analytics] [plan name]"""
    try:
    ...
    while index < len(tokens):
        ...
        if token in {"--autonomous", "-a"}:
            ...
            return (
                "Usage: /pw-pwf [--autonomous|--gated] [--template analytics] [plan name]\n"
    ...
    if not result.get("ok", True):
        return f"planweft: {result.get('error', 'initialization failed')}"
    created = ", ".join(result.get("created") or []) or "none (all present)"
    lines = [
        f"planweft: plan ready ({result.get('plan_id', 'root')}, {result.get('mode', 'legacy')} mode)",
...
def status_command(raw_args: str = "") -> str:
    """/pw-pwf-status and /pw-plan-status: compact planning state for the current project."""
    project_dir = _command_cwd()
    payload = json.loads(planweft_status(cwd=str(project_dir)))
...
def _register_commands(ctx: Any) -> None:
    ...
    try:
        register(
            name="pw-pwf",
            handler=pwf_command,
            description="Start planweft: create the plan files (root or .planning/<slug>), optional --autonomous / --gated",
            args_hint="[--gated] [plan name]",
        )
        register(
            name="pw-pwf-status",
            handler=status_command,
            description="Show the active planweft plan: phase counts, mode, attestation",
        )
        register(
            name="pw-plan-status",
            handler=status_command,
            description="Alias of /pw-pwf-status",
        )
...
def register(ctx: Any) -> None:
    ctx.register_tool(
        ...
        schema={
            "name": "planweft_init",
            "description": (
                "Create planweft markdown files (task_plan.md, findings.md, progress.md) "
        ...
        handler=lambda args, **kw: planweft_init(
            template=args.get("template", "default"),
            cwd=args.get("cwd", ""),
            name=args.get("name", ""),
            mode=args.get("mode", ""),
        ),
        description="Initialize planweft state files.",
    )
    ctx.register_tool(
        name="planweft_status",
        toolset="terminal",
        schema={
            "name": "planweft_status",
        ...
        handler=lambda args, **kw: planweft_status(cwd=args.get("cwd", "")),
        description="Show planweft status summary.",
    )
    ctx.register_tool(
        name="planweft_check_complete",
        toolset="terminal",
        schema={
            "name": "planweft_check_complete",
            "description": "Run the planweft completion check for the active plan.",
            "parameters": {
                "type": "object",
                "properties": {
                    "cwd": {"type": "string", "description": "Target project directory. Defaults to current working directory."},
        ...
        handler=lambda args, **kw: planweft_check_complete(cwd=args.get("cwd", "")),
