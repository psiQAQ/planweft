...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planweft] 未找到 task_plan.md — 沒有進行中的規劃會話。"
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[planweft] 所有階段已完成（$COMPLETE/$TOTAL）。如果使用者有額外工作，請在開始前於 task_plan.md 中新增階段。"
else
    echo "[planweft] 任務進行中（$COMPLETE/$TOTAL 個階段已完成）。停止前請更新 progress.md。"
    if [ "$IN_PROGRESS" -gt 0 ]; then
        echo "[planweft] $IN_PROGRESS 個階段仍在進行中。"
    fi
    if [ "$PENDING" -gt 0 ]; then
        echo "[planweft] $PENDING 個階段待處理。"
    fi
fi
exit 0
