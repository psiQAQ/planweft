...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[program-design] 未找到 task_plan.md — 没有进行中的规划会话。'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[program-design] 所有阶段已完成（' + $COMPLETE + '/' + $TOTAL + '）。如果用户有额外工作，请在开始前于 task_plan.md 中新增阶段。')
} else {
    Write-Host ('[program-design] 任务进行中（' + $COMPLETE + '/' + $TOTAL + ' 个阶段已完成）。停止前请更新 progress.md。')
    if ($IN_PROGRESS -gt 0) {
        Write-Host ('[program-design] ' + $IN_PROGRESS + ' 个阶段仍在进行中。')
    }
    if ($PENDING -gt 0) {
        Write-Host ('[program-design] ' + $PENDING + ' 个阶段待处理。')
    }
}
exit 0
