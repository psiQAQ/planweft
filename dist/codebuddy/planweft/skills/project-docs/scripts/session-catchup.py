#!/usr/bin/env python3
"""
Session Catchup Script for planweft
...
def frame_untrusted_context(kind: str, text: str, limit: int = 65536) -> str:
    ...
    nonce = hashlib.sha256(
        b'planweft-context-v1\0' + kind.encode('ascii') + b'\0' + payload
    ).hexdigest()[:24]
...
def filter_sessions_by_cwd(sessions: List[Path], project_path: str) -> Tuple[List[Path], Optional[str]]:
    ...
    if mine:
        ...
        if unknown:
            notice = (
                "[planweft] Session catchup quarantined "
                f"{len(unknown)} transcript(s) without canonical cwd identity."
            )
        return mine, notice
    if foreign:
        return [], (
            "[planweft] Session catchup skipped: "
...
def emit_metadata_report(runtime_name: str, unsynced_count: int) -> None:
    ...
    print("\n[planweft] SESSION CATCHUP AVAILABLE")
...
def opencode_catchup(project_path: str, mode: str = 'no-history') -> None:
    ...
    print(f"\n[planweft] SESSION CATCHUP DETECTED (IDE: opencode)")
...
def main():
    ...
    print("\n[planweft] SESSION CATCHUP DETECTED")
