...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[program-design] 未找到 task_plan.md — 沒有進行中的規劃會話。'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[program-design] 所有階段已完成（' + $COMPLETE + '/' + $TOTAL + '）。如果使用者有額外工作，請在開始前於 task_plan.md 中新增階段。')
} else {
    Write-Host ('[program-design] 任務進行中（' + $COMPLETE + '/' + $TOTAL + ' 個階段已完成）。停止前請更新 progress.md。')
    if ($IN_PROGRESS -gt 0) {
        Write-Host ('[program-design] ' + $IN_PROGRESS + ' 個階段仍在進行中。')
    }
    if ($PENDING -gt 0) {
        Write-Host ('[program-design] ' + $PENDING + ' 個階段待處理。')
    }
}
exit 0
