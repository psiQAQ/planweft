#!/usr/bin/env python3
"""
Session Catchup Script for program-design
...
def frame_untrusted_context(kind: str, text: str, limit: int = 65536) -> str:
    ...
    nonce = hashlib.sha256(
        b'program-design-context-v1\0' + kind.encode('ascii') + b'\0' + payload
    ).hexdigest()[:24]
...
def filter_sessions_by_cwd(sessions: List[Path], project_path: str) -> Tuple[List[Path], Optional[str]]:
    ...
    if mine:
        ...
        if unknown:
            notice = (
                "[program-design] Session catchup quarantined "
                f"{len(unknown)} transcript(s) without canonical cwd identity."
            )
        return mine, notice
    if foreign:
        return [], (
            "[program-design] Session catchup skipped: "
...
def get_session_candidates(
    ...
    if script_path.endswith('/.codex/skills/project-docs/scripts/session-catchup.py'):
        return 'codex', get_codex_sessions(project_path)
    if script_path.endswith('/.opencode/skills/project-docs/scripts/session-catchup.py'):
...
def emit_metadata_report(runtime_name: str, unsynced_count: int) -> None:
    ...
    print("\n[program-design] SESSION CATCHUP AVAILABLE")
...
def opencode_catchup(project_path: str, mode: str = 'no-history') -> None:
    ...
    print(f"\n[program-design] SESSION CATCHUP DETECTED (IDE: opencode)")
...
def main():
    ...
    print("\n[program-design] SESSION CATCHUP DETECTED")
