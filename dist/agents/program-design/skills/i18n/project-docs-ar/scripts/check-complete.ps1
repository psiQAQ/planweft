...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[program-design-ar] لم يتم العثور على task_plan.md — لا توجد جلسة تخطيط نشطة.'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[program-design-ar] اكتملت جميع المراحل (' + $COMPLETE + '/' + $TOTAL + '). إذا كان لدى المستخدم عمل إضافي، أضف مراحل في task_plan.md قبل البدء.')
} else {
    Write-Host ('[program-design-ar] المهمة قيد التنفيذ (' + $COMPLETE + '/' + $TOTAL + ' مرحلة مكتملة). حدّث progress.md قبل التوقف.')
    if ($IN_PROGRESS -gt 0) {
        Write-Host ('[program-design-ar] ' + $IN_PROGRESS + ' مرحلة/مراحل لا تزال قيد التنفيذ.')
    }
    if ($PENDING -gt 0) {
        Write-Host ('[program-design-ar] ' + $PENDING + ' مرحلة/مراحل معلقة.')
    }
}
exit 0
