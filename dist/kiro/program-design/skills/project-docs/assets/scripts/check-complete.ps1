...
if (-not (Test-Path $PlanFile)) {
    Write-Host "[program-design] No task plan at $PlanFile — run bootstrap or specify path."
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    ...
    Write-Host "[program-design] In progress ($COMPLETE/$TOTAL phases complete)."
    if ($IN_PROGRESS -gt 0) {
        Write-Host "[program-design] $IN_PROGRESS phase(s) in_progress."
    }
    if ($PENDING -gt 0) {
