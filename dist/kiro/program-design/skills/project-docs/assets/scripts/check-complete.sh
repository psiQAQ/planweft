...
if [ ! -f "$PLAN_FILE" ]; then
  echo "[program-design] No task plan at $PLAN_FILE — run bootstrap or specify path."
  exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
  echo "[program-design] ALL PHASES COMPLETE ($COMPLETE/$TOTAL)."
else
  echo "[program-design] In progress ($COMPLETE/$TOTAL phases complete)."
  if [ "$IN_PROGRESS" -gt 0 ]; then
    echo "[program-design] $IN_PROGRESS phase(s) in_progress."
  fi
