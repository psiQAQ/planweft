...
if agent_valid and (agent_id is None or prompt_valid):
    digest = hashlib.sha256(b"planweft-skill-turn-v1\0")
...
case "$EVENT" in
    ...
    posttool)
        ...
        printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PostToolUse","additionalContext":"[planweft] Update progress.md with what you just did. If a phase is now complete, update task_plan.md status."}}'
        ;;
    precompact)
