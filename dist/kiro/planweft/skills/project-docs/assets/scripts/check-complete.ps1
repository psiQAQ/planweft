...
if (-not (Test-Path $PlanFile)) {
    Write-Host "[planweft] No task plan at $PlanFile — run bootstrap or specify path."
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    ...
    Write-Host "[planweft] In progress ($COMPLETE/$TOTAL phases complete)."
    if ($IN_PROGRESS -gt 0) {
        Write-Host "[planweft] $IN_PROGRESS phase(s) in_progress."
    }
    if ($PENDING -gt 0) {
