...
if [ ! -f "$PLAN_FILE" ]; then
  echo "[planweft] No task plan at $PLAN_FILE — run bootstrap or specify path."
  exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
  echo "[planweft] ALL PHASES COMPLETE ($COMPLETE/$TOTAL)."
else
  echo "[planweft] In progress ($COMPLETE/$TOTAL phases complete)."
  if [ "$IN_PROGRESS" -gt 0 ]; then
    echo "[planweft] $IN_PROGRESS phase(s) in_progress."
  fi
