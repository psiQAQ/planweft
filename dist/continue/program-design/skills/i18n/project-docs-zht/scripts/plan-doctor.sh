#!/bin/sh
# program-design: plan-doctor — one-pass self-check for the mechanisms
...
echo '=== program-design plan-doctor ==='
info "cwd: ${PWD}"
    ...
    if [ -z "${OUT}" ]; then
        ...
        case "${OUT}" in
            *'===BEGIN-PWF-DATA'*)
                ...
                ;;
            *'[PLAN TAMPERED'*)
                warn "injection: plan is attested but the hash mismatches — run /pd-plan-attest (or scripts/attest-plan.sh) to re-approve the current plan"
                ;;
...
if [ -n "${ATT}" ]; then
    ...
    info "attestation: none (opt-in in legacy mode; default-on in v3 modes; run /pd-plan-attest after approving the plan)"
fi
...
for s in \
    ...
    "${HOME:-}/.claude/skills/project-docs" \
    ".agents/skills/project-docs" \
    "${HOME:-}/.agents/skills/project-docs"
do
...
# Program Design addition: existence is only a diagnostic, never activation.
