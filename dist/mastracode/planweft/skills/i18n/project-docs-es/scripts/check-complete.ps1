...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[planweft-es] No se encontró task_plan.md — no hay sesión de planificación activa.'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[planweft-es] Todas las fases completadas (' + $COMPLETE + '/' + $TOTAL + '). Si el usuario tiene trabajo adicional, añadir fases en task_plan.md antes de comenzar.')
} else {
    Write-Host ('[planweft-es] Tarea en progreso (' + $COMPLETE + '/' + $TOTAL + ' fases completadas). Actualizar progress.md antes de detenerse.')
    if ($IN_PROGRESS -gt 0) {
        Write-Host ('[planweft-es] ' + $IN_PROGRESS + ' fases aún en progreso.')
    }
    if ($PENDING -gt 0) {
        Write-Host ('[planweft-es] ' + $PENDING + ' fases pendientes.')
    }
}
exit 0
