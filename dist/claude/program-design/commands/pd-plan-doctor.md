---
description: "Self-check for the program-design mechanisms that fail silently: plan resolution, hook injection, canonicalizer path shape, attestation state, install surfaces, and per-fire hook latency. Run it whenever hooks seem quiet or after installing on a new machine. Available since v3.6.0."
...
Run the program-design self-check against the current project directory.
...
3. If any line reports FAIL, explain the matching remediation:
   ...
   - tamper WARN → re-approve with `/pd-plan-attest` if the plan edit was intentional.
