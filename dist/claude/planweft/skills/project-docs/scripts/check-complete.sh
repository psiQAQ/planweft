...
if [ -n "${PLAN_FILE}" ]; then
    ...
    if [ -n "${RESOLVED_DIR}" ] && [ -f "${RESOLVED_DIR}/task_plan.md" ]; then
        ...
        echo "[planweft] An explicit PLAN_ID or PWF_PLAN_ROOT did not resolve to a plan; no completion state was read and no other plan was substituted."
        exit 0
    else
...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planweft] No task_plan.md found — no active planning session."
    exit 0
fi
...
advisory_report() {
    if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
        echo "[planweft] ALL PHASES COMPLETE ($COMPLETE/$TOTAL). If the user has additional work, add new phases to task_plan.md before starting."
    else
        echo "[planweft] Task in progress ($COMPLETE/$TOTAL phases complete). Update progress.md before stopping."
        if [ "$IN_PROGRESS" -gt 0 ]; then
            echo "[planweft] $IN_PROGRESS phase(s) still in progress."
        fi
        if [ "$PENDING" -gt 0 ]; then
            echo "[planweft] $PENDING phase(s) pending."
        fi
    fi
}
...
if [ "${BLOCKS}" -ge "${CAP}" ]; then
    advisory_report
    echo "[planweft] gate cap reached ($BLOCKS/$CAP) — allowing stop."
    exit 0
fi
...
if [ "${BLOCKS}" -gt 0 ] && [ "${LEDGER_NOW}" -eq "${LEDGER_PREV}" ]; then
    advisory_report
    echo "[planweft] no progress since last gate block — allowing stop."
    exit 0
fi
...
printf '{"decision":"block","reason":"[planweft] Gated plan incomplete: phase '\''%s'\'' is in_progress (%s/%s complete, gate block %s/%s). Finish or update the plan, then stop."}\n' \
