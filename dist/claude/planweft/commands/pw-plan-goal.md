...
3. Derive a goal condition from the plan content:
   ...
   - If user passed an argument: use that as an additional clause (e.g., `/pw-plan-goal until all tests pass`)
...
- `/pw-plan-goal` does not replace `/goal`. It composes with it. Users can still run `/goal "any text"` directly.
...
- Combine with `/pw-plan-loop` for a "babysit until done" workflow: `/pw-plan-loop` cadence + `/pw-plan-goal` termination.
