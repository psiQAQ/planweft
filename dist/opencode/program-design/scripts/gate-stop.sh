#!/bin/sh
# program-design: Stop-hook dispatcher for the v3 completion gate.
#
...
if [ ! -f "$TARGET" ] && [ -n "${HOME:-}" ]; then
    ...
    TARGET=$(ls "${HOME}/.claude/skills/project-docs/scripts/check-complete.sh" \
                "${HOME}/.claude/plugins/marketplaces/program-design/scripts/check-complete.sh" \
                2>/dev/null | head -1)
fi
