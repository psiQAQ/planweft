...
export const PlanningWithFiles: Plugin = async ({ client, directory }) => {
  ...
  function toolRoot(project: string): string | { ok: false; error: string } {
    if (env.PLANNING_DISABLED === "1") return { ok: false, error: "PLANNING_DISABLED=1 is set for this session; planweft is switched off." }
  ...
  return {
    ...
    tool: {
      pw_init: tool({
        description:
          "planweft: create task_plan.md, findings.md and progress.md. A name creates an isolated .planning/YYYY-MM-DD-<slug>/ plan and makes it active; mode autonomous or gated writes the v3 markers and attests the plan.",
        args: {
