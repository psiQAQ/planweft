...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planweft-ar] لم يتم العثور على task_plan.md — لا توجد جلسة تخطيط نشطة."
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[planweft-ar] اكتملت جميع المراحل ($COMPLETE/$TOTAL). إذا كان لدى المستخدم عمل إضافي، أضف مراحل في task_plan.md قبل البدء."
else
    echo "[planweft-ar] المهمة قيد التنفيذ ($COMPLETE/$TOTAL مرحلة مكتملة). حدّث progress.md قبل التوقف."
    if [ "$IN_PROGRESS" -gt 0 ]; then
        echo "[planweft-ar] $IN_PROGRESS مرحلة/مراحل لا تزال قيد التنفيذ."
    fi
    if [ "$PENDING" -gt 0 ]; then
        echo "[planweft-ar] $PENDING مرحلة/مراحل معلقة."
    fi
fi
exit 0
