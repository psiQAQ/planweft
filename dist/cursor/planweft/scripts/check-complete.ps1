...
if (-not (Test-Path $PlanFile)) {
    Write-Host '[planweft] No task_plan.md found -- no active planning session.'
    exit 0
}
...
function Write-AdvisoryReport {
    if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
        Write-Host ('[planweft] ALL PHASES COMPLETE (' + $COMPLETE + '/' + $TOTAL + '). If the user has additional work, add new phases to task_plan.md before starting.')
    } else {
        Write-Host ('[planweft] Task in progress (' + $COMPLETE + '/' + $TOTAL + ' phases complete). Update progress.md before stopping.')
        if ($IN_PROGRESS -gt 0) {
            Write-Host ('[planweft] ' + $IN_PROGRESS + ' phase(s) still in progress.')
        }
        if ($PENDING -gt 0) {
            Write-Host ('[planweft] ' + $PENDING + ' phase(s) pending.')
        }
    }
}
...
if ($blocks -ge $cap) {
    Write-AdvisoryReport
    Write-Host ('[planweft] gate cap reached (' + $blocks + '/' + $cap + ') -- allowing stop.')
    exit 0
}
...
if ($blocks -gt 0 -and $ledgerNow -eq $ledgerPrev) {
    Write-AdvisoryReport
    Write-Host '[planweft] no progress since last gate block -- allowing stop.'
    exit 0
}
...
$reason = "[planweft] Gated plan incomplete: phase '" + $phaseEscaped + "' is in_progress (" + $COMPLETE + "/" + $TOTAL + " complete, gate block " + $newBlocks + "/" + $cap + "). Finish or update the plan, then stop."
