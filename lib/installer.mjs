  ...
  detectDuplicates(host, old) {
    ...
    if (host === 'dsh' && !(old?.skillOnly ?? this.o.skillOnly)) {
      ...
      for (const file of [path.join(root, 'cordis.patch.yml'), path.join(root, 'profiles', profile, 'cordis.patch.yml')]) {
        ...
        if (/planning-with-files|program-design|planweft\/dsh|dsh-hook\.sh/.test(text)) throw Error(`Another planning hook may be active in ${file}; inspect it before installing`);
      }
    }
