...
export const HOSTS = ['codex', 'claude', 'pi', 'opencode', 'hermes', 'cursor', 'gemini', 'copilot', 'mastracode', 'kiro', 'continue', 'factory', 'codebuddy', 'agents', 'dsh'];
...
const planningRegistration = value => /planning-with-files|program-design|npm:planweft|["']planweft@|^planweft@|planweft-cli-|[/\\]planweft(?:[/\\"']|$)/.test(value);
...
export function parse(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (['-a', '--agent', '--source', '--dsh-profile'].includes(a)) {
      const v = args[++i]; if (!v || v.startsWith('-')) throw Error(`Missing value for ${a}`);
      if (a === '--dsh-profile') { if (o.dshProfile || !/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/.test(v) || v === 'desktop') throw Error('Invalid or duplicate DSH profile'); o.dshProfile = v; }
      else if (a === '--source') { if (o.source) throw Error('Duplicate --source'); o.source = path.resolve(v); }
  ...
  o.explicit = [...seen];
  if (o.dshProfile && !o.agents.includes('dsh')) throw Error('--dsh-profile requires -a dsh');
...
export class Installer {
  ...
  hostRoot(host, scope = this.o.scope) {
    const user = scope === 'global', root = user ? this.home : this.cwd;
    if (host === 'dsh') {
      if (user) {
        const value = this.env.DSH_HOME?.trim() ? this.env.DSH_HOME : path.join(root, '.dsh');
        return path.resolve(value === '~' ? this.home : /^~[\\/]/.test(value) ? path.join(this.home, value.slice(2)) : value);
      }
      // DSH scans the nearest Git root, not necessarily the invoking directory.
      for (let dir = root; ; dir = path.dirname(dir)) {
        if (exists(path.join(dir, '.git'))) return path.join(dir, '.dsh');
        if (path.dirname(dir) === dir) return path.join(root, '.dsh');
      }
    }
  ...
  components(host, root, opt) {
    ...
    if (host === 'dsh') return opt.skillOnly ? [{source: path.join(root, 'dist/dsh/planweft/skills/project-docs'), target: skill}] : [];
    ...
    if (host === 'opencode') {
      const commands = path.join(root, 'dist/opencode/planweft/commands');
      const extras = exists(commands) ? fs.readdirSync(commands).filter(n => n.endsWith('.md')).map(n => ({text: fs.readFileSync(path.join(commands, n), 'utf8'), target: path.join(out, 'commands', n)})) : [];
      return [{source: path.join(root, 'dist/opencode/planweft/skills/project-docs'), target: skill}, {text: `// Managed by PlanWeft; update through its installer.\nexport { PlanningWithFiles } from ${JSON.stringify(pathToFileURL(path.join(root, 'dist/opencode/planweft/dist/index.js')).href)};\n`, target: path.join(out, 'plugins/planweft.ts')}, ...extras];
    }
    return [];
  }
  assertOwned(item) {
  ...
  assertRecord(host, rec) {
    ...
    if (host === 'dsh' && !rec.skillOnly) this.assertDshRegistration(rec);
  ...
  checkCodexRegistrations(file, old) {
    const registered = old?.steps?.['marketplace-add'] === 'done';
    if (old && !old.skillOnly) {
      if (this.o.scope !== 'global' || old.scope !== 'global') throw Error('Unexpected Codex native registration scope');
      if (old.status === 'installed' && (!registered || old.steps?.['native-install'] !== 'done')) throw Error('Inconsistent completed Codex registration steps');
    }
    if (!exists(file)) {
      if (registered) throw Error('Owned Codex registration missing; native configuration preserved');
      return;
    }
    let config;
    try {
      // Parse, never rewrite. Fatal decoding prevents invalid bytes from being
      // silently replaced before identity comparison. Parser errors can quote
      // config values, so do not include their contents in diagnostics.
      const text = new TextDecoder('utf-8', {fatal: true}).decode(fs.readFileSync(file));
      config = createRequire(import.meta.url)('toml').parse(text);
    } catch { throw Error(`Cannot read Codex configuration as UTF-8 TOML: ${file}`); }
    const table = value => value && typeof value === 'object' && !Array.isArray(value) && !(value instanceof Date);
    for (const key of ['marketplaces','plugins']) if (config[key] !== undefined && !table(config[key])) throw Error(`Invalid Codex ${key} table; configuration preserved`);
    const registry = path.join(this.base, 'registries', 'codex');
    const localSource = source => typeof source === 'string' && path.isAbsolute(source) && path.resolve(source) === registry;
    if (registered) {
      if (old.skillOnly || old.catalog !== this.catalog('codex') || old.nativeId !== `planweft@${old.catalog}`) throw Error('Unexpected Codex registration identity');
      const catalogFile = path.join(registry, catalogs.codex), payload = path.join(registry, 'payload');
      noLinks(catalogFile); noLinks(payload);
      if (!exists(catalogFile) || !exists(payload) || hash(fs.readFileSync(catalogFile)) !== old.registrySha256 || digest(payload) !== old.payloadSha256) throw Error('User-modified or missing CLI marketplace preserved');
      const market = config.marketplaces?.[old.catalog];
      if (!table(market) || market.source_type !== 'local' || !localSource(market.source)) throw Error('Owned Codex marketplace source changed or missing; configuration preserved');
      // A successful marketplace step can survive a failed plugin install.
      // Its owned source can be reused on retry, but doctor still checks the
      // final receipt status and must not report partial installation success.
      if (old.steps['native-install'] === 'done' || old.status === 'installed') {
        const plugin = config.plugins?.[old.nativeId];
        if (!table(plugin) || plugin.enabled !== true) throw Error('Owned Codex plugin missing or disabled; configuration preserved');
      }
    }
    for (const [name, market] of Object.entries(config.marketplaces || {})) {
      if (name !== (registered ? old.catalog : null) && localSource(market?.source)) throw Error('Another Codex marketplace uses the managed source; configuration preserved');
    }
    const scan = (value, location = []) => {
      if (typeof value === 'string' && planningRegistration(value)) return true;
      if (!value || typeof value !== 'object') return false;
      return Object.entries(value).some(([key, entry]) => {
        // Skip only owned identity keys and the single verified source value.
        // Keep all other fields, including values inside the owned plugin.
        const identity = registered && location.length === 1 &&
          ((location[0] === 'marketplaces' && key === old.catalog) || (location[0] === 'plugins' && key === old.nativeId));
        const source = registered && location.length === 2 && location[0] === 'marketplaces' && location[1] === old.catalog && key === 'source';
        return (!identity && planningRegistration(key)) || (!source && scan(entry, [...location, key]));
      });
    };
    if (scan(config)) throw Error(`Another planning registration may be active in ${file}; inspect it before installing`);
  }
  detectDuplicates(host, old) {
    ...
    if (host === 'dsh' && !(old?.skillOnly ?? this.o.skillOnly)) {
      const profile = this.o.dshProfile || old?.dshProfile || 'headless';
      for (const file of [path.join(root, 'cordis.patch.yml'), path.join(root, 'profiles', profile, 'cordis.patch.yml')]) {
        noLinks(file);
        const text = exists(file) ? fs.readFileSync(file, 'utf8').split('\n').filter(line => !/^\s*#/.test(line)).join('\n') : '';
        if (/planning-with-files|program-design|planweft\/dsh|planweft[\/\\].*index\.mjs|id:\s*planweft\b|dsh-hook\.sh/.test(text)) throw Error(`Another planning hook may be active in ${file}; inspect it before installing`);
      }
    }
    ...
    // These hosts merge user and project registrations. A project receipt owns
    // only its project entry; the same source in user settings is still foreign.
    const foreignChecks = new Set();
    if (['pi', 'opencode'].includes(host)) {
      const otherRoot = this.hostRoot(host, this.o.scope === 'project' ? 'global' : 'project');
      // Local native loaders need not appear in package settings at all.
      // Only known PlanWeft identities are checked; arbitrary aliases still
      // require native discovery/doctor review, not guessing at user files.
      for (const name of host === 'opencode' ? ['plugins/planweft.ts', 'plugins/planweft.js', 'plugins/planweft.mjs'] : ['extensions/planweft.ts', 'extensions/planweft/index.ts']) {
        const target = path.join(otherRoot, name);
        if (exists(target)) {
          // preflight/doctor validated old via assertRecord. A configured root
          // alias can expose that same owned file; do not exempt other loaders.
          const owned = (old?.components || []).some(item => exists(item.target)
            && fs.realpathSync(item.target) === fs.realpathSync(target));
          if (!owned) throw Error(`Another planning native loader may be active in ${target}; inspect it before installing`);
        }
      }
      for (const name of host === 'pi' ? ['settings.json'] : ['opencode.json', 'opencode.jsonc']) {
        const file = path.resolve(otherRoot, name);
        if (!checks.some(current => path.resolve(current) === file || (exists(current) && exists(file)
          && fs.realpathSync(current) === fs.realpathSync(file)))) { checks.push(file); foreignChecks.add(file); }
      }
    }
    for (const file of checks) {
      if (host === 'codex') { this.checkCodexRegistrations(file, old); continue; }
      ...
      // Skill-only pairing adds no second runtime. Require the exact CLI
      // version so a native npm plugin cannot silently use a mismatched Skill.
      if (host === 'opencode' && (old?.skillOnly ?? this.o.skillOnly)) {
        text = text.split(JSON.stringify(`planweft@${this.pkg.version}`)).join('');
      }
      if (host === 'pi' && !foreignChecks.has(file) && old?.nativeSource) {
        // Pi persists project sources relative to the settings file, not cwd.
        // Exempt one owned package source only; duplicate entries and foreign
        // scope registrations must remain visible to the check below.
        // Match Pi's native settings reader; a leading UTF-8 BOM is valid.
        const settings = JSON.parse(text.replace(/^\uFEFF/, '')); let removed = false;
        if (Array.isArray(settings.packages)) settings.packages = settings.packages.map(entry => {
          const source = typeof entry === 'string' ? entry : entry?.source;
          if (removed || typeof source !== 'string') return entry;
          const local = path.isAbsolute(source) || /^\.{1,2}[\\/]/.test(source) || /^~[\\/]/.test(source);
          const resolved = /^~[\\/]/.test(source) ? path.resolve(this.home, source.slice(2))
            : local ? path.resolve(path.dirname(file), source) : null;
          if (source !== old.nativeSource && resolved !== path.resolve(old.nativeSource)) return entry;
          removed = true;
          if (typeof entry === 'string') return null;
          const rest = {...entry}; delete rest.source; return rest;
        });
        text = JSON.stringify(settings);
      }
      if (!foreignChecks.has(file)) {
        if (old?.nativeId) text = text.split(old.nativeId).join('');
        if (host !== 'pi' && old?.nativeSource) {
          // Native JSON escapes Windows separators and quotes in path names.
          text = text.split(JSON.stringify(old.nativeSource)).join('');
          text = text.split(old.nativeSource).join('');
        }
        text = text.split(`${old?.catalog || this.catalog(host)}`).join('');
      }
      if (planningRegistration(text)) throw Error(`Another planning registration may be active in ${file}; inspect it before installing`);
    }
  }
  preflight(host) {
    ...
    if (host === 'dsh' && !opt.skillOnly) {
      if (this.o.scope !== 'global') throw Error('DSH profiles use user scope; select --global or --skill-only');
      const profile = this.o.dshProfile || old?.dshProfile || 'headless';
      if (old?.dshProfile && profile !== old.dshProfile) throw Error('Remove the existing DSH integration before changing profile');
      const file = path.join(this.hostRoot('dsh'), 'profiles', profile, 'package.json');
      noLinks(file);
      if (exists(file)) {
        const manifest = json(file);
        if (!old && (manifest.dependencies?.planweft || manifest.dsh?.profile?.bundles?.includes('planweft'))) throw Error('Foreign DSH PlanWeft registration preserved');
      }
    }
    if (host === 'dsh' && this.o.scope === 'project' && this.hostRoot(host) !== path.join(this.cwd, '.dsh')) throw Error(`Run DSH project installation from its Git root: ${path.dirname(this.hostRoot(host))}`);
  ...
  place(c, method) {
    ...
    if (method !== 'copy' && c.text === undefined) {
      try {
    ...
    try {
      ...
      if (c.text !== undefined) fs.writeFileSync(item, c.text, {flag: 'wx'});
      else fs.cpSync(c.source, item, {recursive: true, errorOnExist: true, force: false});
  ...
  dshManifest(rec) {
    if (this.o.dshProfile && this.o.dshProfile !== rec.dshProfile) throw Error('Explicit DSH profile differs from the managed profile');
    if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/.test(rec.dshProfile || '') || rec.dshProfile === 'desktop' || rec.dshHome !== this.hostRoot('dsh')) throw Error('DSH profile identity changed');
    const file = path.join(rec.dshHome, 'profiles', rec.dshProfile, 'package.json');
    noLinks(file);
    return exists(file) ? json(file) : {};
  }
  assertDshRegistration(rec) {
    const manifest = this.dshManifest(rec), actual = manifest.dependencies?.planweft;
    if (!rec.nativeSource?.startsWith(path.join(this.base, 'versions') + path.sep)) throw Error('Unexpected DSH source');
    if (rec.nativeSource !== rec.packageRoot) this.checkStore(rec.nativeSource);
    const matches = typeof actual === 'string' && actual.startsWith('link:') &&
      path.resolve(rec.dshHome, 'profiles', rec.dshProfile, actual.slice(5)) === rec.nativeSource;
    if (actual && !matches) throw Error('User-modified DSH registration preserved');
    if (rec.steps['native-install'] === 'done' && (!matches || manifest.dsh?.profile?.bundles?.filter(n => n === 'planweft').length !== 1)) throw Error('DSH native registration missing or duplicated');
  }
  ...
  native(host, rec, old) {
    ...
    noLinks(this.hostRoot(host)); fs.mkdirSync(this.hostRoot(host), {recursive: true});
    if (host === 'dsh') {
      rec.dshProfile = this.o.dshProfile || old?.dshProfile || 'headless';
      rec.dshHome = this.hostRoot(host); rec.nativeSource = rec.packageRoot;
      rec.nativeRegistration = 'link:' + rec.nativeSource; this.save();
      try {
        this.step(rec, 'native-install', () => this.exec(['dsh', 'plugin', '--profile', rec.dshProfile, 'add', rec.nativeRegistration]));
        const actual = this.dshManifest(rec).dependencies?.planweft;
        // pnpm normalizes a link to a relative profile path.
        if (!actual?.startsWith('link:') || path.resolve(rec.dshHome, 'profiles', rec.dshProfile, actual.slice(5)) !== rec.nativeSource) throw Error('DSH installed an unexpected source');
        rec.nativeRegistration = actual; this.save(); this.assertDshRegistration(rec);
      } catch (error) {
        if (old?.nativeRegistration) {
          this.step(rec, 'native-restore', () => this.exec(['dsh', 'plugin', '--profile', rec.dshProfile, 'add', 'link:' + old.nativeSource]));
          rec.nativeSource = old.nativeSource; rec.nativeRegistration = old.nativeRegistration; rec.actualVersion = old.version; this.save();
        }
        throw error;
      }
      return;
    ...
    if (!old?.steps?.['marketplace-add'] || old.steps['marketplace-add'] !== 'done') this.step(rec, 'marketplace-add', () => this.exec([cli, 'plugin', 'marketplace', 'add', registry]));
      ...
      rec.steps['marketplace-add'] = 'done';
      // Codex upgrade refreshes Git snapshots only. A local catalog is read by
      // plugin add again; invoking another host's marketplace update is invalid.
      if (host !== 'codex') this.step(rec, 'marketplace-update', () => this.exec([cli, 'plugin', 'marketplace', 'update', rec.catalog]));
    }
  ...
  remove(host) {
    ...
    if (['done', 'failed', 'in-progress'].includes(rec.steps['native-install'])) {
      const cli = executables[host];
      const args = host === 'dsh' ? ['dsh', 'plugin', '--profile', rec.dshProfile, 'remove', 'planweft'] : host === 'pi' ? ['pi', 'remove', ...this.piScope(), rec.nativeSource] : host === 'gemini' ? ['gemini', 'extensions', 'uninstall', 'planweft'] : [cli, 'plugin', host === 'codex' ? 'remove' : 'uninstall', rec.nativeId, ...(['claude', 'factory', 'codebuddy'].includes(host) ? ['--scope', scope] : [])];
  ...
  async execute() {
    const o = this.o;
    if (['help', '--help', '-h'].includes(o.action)) { this.log('planweft <add|update|remove|list|doctor> -a HOST [-a HOST] [--project|--global] [--skill-only] [--copy|--symlink] [--dry-run] [--source package.tgz] [--approve-pi-project] [--dsh-profile NAME]\nHosts: ' + HOSTS.join(', ') + '\nDefault: project scope, full native integration, symlink with reported copy fallback. Update uses this CLI version.'); return 0; }
    if (['list', 'doctor'].includes(o.action)) {
      ...
      for (const [host, rec] of Object.entries(this.state.agents)) {
        ...
        if (host === 'dsh' && !rec.skillOnly && o.dshProfile && o.dshProfile !== rec.dshProfile) throw Error('Explicit DSH profile differs from the managed profile');
