...
def records_unchanged(project, protected):
    try:
        return all((project/name).is_file() and not (project/name).is_symlink()
                   and (project/name).read_bytes()==value.encode() for name,value in protected.items())
    except OSError:
        return False
...
def main(argv=None):
    ...
    env.update(HOME=str(home), USERPROFILE=str(home), XDG_CONFIG_HOME=str(home / '.config'),
               ...
               npm_config_userconfig=os.devnull, PLANNING_DISABLED='1', TMPDIR=str(temporary),
    ...
    def run(label, argv, cwd=project, input_data='y\ny\ny\n'):
        try:
            ...
            records.append({'step':label,'argv':argv,'error':'TimeoutExpired'})
            raise
    ...
    archives = {}
    status = 'Failed'
    (out / 'summary.json').write_text(json.dumps({'status':'In Progress','host':args.host,'commands':records}))
    try:
                ...
                try:
                    ...
                    if proc.poll() is None:
                        ...
                        try: stdout, stderr = proc.communicate(timeout=5)
                            ...
                            proc.kill(); stdout, stderr = proc.communicate(timeout=5)
        ...
        if not records_unchanged(project,protected):raise RuntimeError('Project record changed or missing')
        status = 'Passed'
    except (Exception, KeyboardInterrupt) as error:
        status = 'Failed'; records.append({'error': str(error) or type(error).__name__})
    (out / 'summary.json').write_text(json.dumps({'status': status, 'host': args.host, 'commands': records,
        ...
        'model_session': 'Not Run: installer lifecycle only', 'fixture_archives': {label: {'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'path':str(path)} for label,path in archives.items() if path.is_file()},
        'project_records_unchanged': records_unchanged(project,protected)}, indent=2))
